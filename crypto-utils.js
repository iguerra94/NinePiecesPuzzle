export const sha256 = (message) => {
	...
	message += '\x80' // Append Ƈ' bit (plus zero padding)
	while (message[lengthProperty]%64 - 56) message += '\x00' // More zero padding
	for (i = 0; i < message[lengthProperty]; i++) {
		j = message.charCodeAt(i);
		if (j>>8) return; // message check: only accept characters in range 0-255
		words[i>>2] |= j << ((3 - i)%4)*8;
	}
